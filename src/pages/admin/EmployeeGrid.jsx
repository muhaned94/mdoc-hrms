        ...
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    ...
                    <p className="text-slate-500 dark:text-slate-400 text-sm">عرض تفصيلي لجميع بيانات الموظفين - يمكنك الضغط على عناوين الجدول للترتيب</p>
                </div>
                <div className="flex gap-2">
                    <button
                        onClick={handleExport}
                        className="flex items-center gap-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-green-600 dark:text-green-500 hover:bg-green-50 dark:hover:bg-green-900/20 px-4 py-2 rounded-lg transition-colors"
                    >
                        <Download size={20} />
            ...
            {/* Search & Filter Bar */}
            <div className="bg-white dark:bg-slate-800 p-4 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 space-y-4 md:space-y-0 md:flex md:items-center md:gap-4">
                <div className="relative flex-1">
                    <input
                        ...
                        className="w-full pl-4 pr-10 py-2 rounded-lg border border-slate-200 dark:border-slate-600 focus:outline-none focus:border-primary dark:bg-slate-700 dark:text-white"
                    />
                ...
                <div className="flex gap-2 w-full md:w-auto overflow-x-auto pb-2 md:pb-0">
                    <div className="relative min-w-[150px]">
                        <select
                            ...
                            className="w-full appearance-none pr-10 pl-4 py-2 bg-slate-50 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-lg text-sm focus:outline-none focus:border-primary dark:text-white"
                        >
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-sm text-right whitespace-nowrap print-fit-table">
                        <thead className="bg-slate-50 dark:bg-slate-700/50 text-slate-600 dark:text-slate-300 font-bold border-b border-slate-200 dark:border-slate-700">
                            <tr>
                                <th className="p-4 border-l border-slate-100 dark:border-slate-700">م</th>
                        ...
                        <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                            {filteredEmployees.map((emp, index) => (
                                <tr key={emp.id} className="hover:bg-blue-50/50 dark:hover:bg-slate-700/30 transition-colors">
                                    <td className="p-4 text-center text-slate-400 border-l border-slate-100 dark:border-slate-700">{index + 1}</td>
                                    <td className="p-4 text-slate-600 dark:text-slate-300 font-mono border-l border-slate-100 dark:border-slate-700 font-bold">{emp.company_id}</td>
                                    <td className="p-4 font-bold text-slate-800 dark:text-white border-l border-slate-100 dark:border-slate-700">
                                        <Link to={`/admin/employees/${emp.id}`} className="flex items-center gap-2 hover:text-primary transition-colors group">
                                            <img src={emp.avatar_url || `https://ui-avatars.com/api/?name=${emp.full_name}&background=random`} className="w-8 h-8 rounded-full border-2 border-white dark:border-slate-700 shadow-sm group-hover:border-primary transition-colors" />
                                            <span className="group-hover:underline">{emp.full_name}</span>
                                        </Link>
                                    </td>
                                    <td className="p-4 text-slate-700 dark:text-slate-300 border-l border-slate-100 dark:border-slate-700">{emp.position || '-'}</td>
                                    <td className="p-4 text-blue-600 dark:text-blue-400 font-medium border-l border-slate-100 dark:border-slate-700">{emp.job_title}</td>
                                    <td className="p-4 text-slate-600 dark:text-slate-300 border-l border-slate-100 dark:border-slate-700">{emp.work_location}</td>
                                    <td className="p-4 text-slate-600 dark:text-slate-300 border-l border-slate-100 dark:border-slate-700">{emp.address || '-'}</td>
                                    <td className="p-4 text-slate-600 dark:text-slate-300 font-mono border-l border-slate-100 dark:border-slate-700 direction-ltr">{emp.phone_number || '-'}</td>
                                    <td className="p-4 text-slate-600 dark:text-slate-300 font-mono border-l border-slate-100 dark:border-slate-700">{emp.email || '-'}</td>
                                    <td className="p-4 border-l border-slate-100 dark:border-slate-700">
                                        <span className={`px-2 py-0.5 rounded-full text-xs font-bold ${emp.work_schedule === 'morning' ? 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400' : 'bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-400'}`}>
                                    ...
                                    <td className="p-4 border-l border-slate-100 dark:border-slate-700 text-slate-500 dark:text-slate-400">{emp.specialization}</td>
                                    <td className="p-4 text-slate-500 dark:text-slate-400">{formatDate(emp.birth_date)}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
                </div>
                <div className="p-4 border-t border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-700/50 text-xs text-slate-500 dark:text-slate-400 flex justify-between print:hidden">
